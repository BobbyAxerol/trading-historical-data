#!/usr/bin/env bash
# Launch exactly the reviewed, detached Phase D BTCUSDT futures-metrics rebuild.
set -euo pipefail
...
hmd_service="phase-d-binance-futures-metrics-5m"
...
exec sudo -n env \
  ...
  PRIMUS_HMD_PHASE_D_BINANCE_FUTURES_METRICS_5M_APPROVED=approved \
  docker compose \
