...
- The only post-B0 staged writer currently approved by this session is the exact `crypto-1m-live` invocation (`collectors.crypto_1m --mode live`). Its protected approval is service-scoped; never introduce or enable a blanket writer approval flag, and do not start another collector under that approval.
