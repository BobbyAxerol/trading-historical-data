...
- The only post-B0 staged writer currently approved by this session is the exact `crypto-1m-live` invocation (`collectors.crypto_1m --mode live --symbols BTCUSDT`). Its protected approval is service-scoped; never introduce or enable a blanket writer approval flag, never rely on the collector's default symbol list, and do not start another collector under that approval.
